...
* We can think of this as *local* (or *case-wise*) *variable importance*
  ...
  - More generally referred to as *prediction explanations* or *feature contributions*  
...
* Conveys how each features value contributed to prediction - *baseline* (or model performance)
...
The Shapley value is the average marginal contribution of a .forestgreen[*player*] across all possible .forestgreen[*coalitions*] in a .MediumSeaGreen[*game*]:
...
* .forestgreen[**Game**] = prediction task for a single observation $\boldsymbol{x}_0$
* .forestgreen[**Gain**] = prediction for $\boldsymbol{x}_0$ minus the average prediction for all training observations
* .forestgreen[**Players**] = the feature values of $\boldsymbol{x}_0$ that collaborate to receive the gain (i.e., predict a certain value)
...
<!-- <img src="images/moes-scenario-matrix.png" style="width: 80%" /> -->
...
* Explaining $N$ instances with $p$ features would require $2 \times m \times N \times p$ calls to $\hat{f}\left(\boldsymbol{b}_1\right) - \hat{f}\left(\boldsymbol{b}_2\right)$ 
* [fastshap](https://cran.r-project.org/package=fastshap) reduces this to $2 \times m \times p$
  ...
  - It's also parallelized across predictors
  
  - Supports Tree SHAP implementation in [xgboost](https://cran.r-project.org/package=xgboost) and [lightgbm](https://cran.r-project.org/package=lightgbm) packages (woot!)
